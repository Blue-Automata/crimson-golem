...
# WIP
# Will eventually be used as debugging info for tests as this will allow us to
# growl messages that describe what action is being taken.
class Wrapper < Selenium::WebDriver::Support::AbstractEventListener
  ...
  def before_find(_by, what, driver)
    display_growl(driver, "Finding element #{what}")
  end

  def after_find(_by, what, driver)
  ...
  def after_click(_element, driver)
    return if @pre_click_url == driver.current_url
  ...
  def display_growl(driver, message)
    driver.execute_script(
      "$.growl({ title: 'Selenium', message: '#{message}' });"
    )
    sleep 0.5
  end
end
